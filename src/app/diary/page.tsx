...
export default function DiaryListPage() {
  ...
  return (
    <div className="diary-list max-w-4xl mx-auto p-4">
      <h1 className="text-3xl font-bold text-center mb-8">다이어리</h1>
      <div className="entries grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div
              ...
              <Link href={`/diary/${entry.id}`} className="block">
                <div>
                  ...
                  <h2 className="text-xl font-semibold mb-2">{entry.title || '제목기능 전에 쓴 글'}</h2>
      ...
      <div className="pagination flex justify-center mt-8">
        {Array.from({ length: Math.ceil(entries.length / entriesPerPage) }, (_, index) => (
          <button
            key={index + 1}
            onClick={() => paginate(index + 1)}
            className={`mx-1 px-4 py-2 rounded ${currentPage === index + 1 ? 'bg-lime-700 text-white' : 'bg-gray-200 text-gray-600'} hover:bg-lime-600 transition-colors`}
          >
            {index + 1}
          </button>
        ))}
